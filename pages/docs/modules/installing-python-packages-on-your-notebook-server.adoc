...
.Verification
* Confirm that the packages in `requirements.txt` appear in the list of packages installed on the notebook server. See link:{rhodsdocshome}{default-format-url}/working_on_data_science_projects/working-on-data-science-projects_nb-server#viewing-python-packages-installed-on-your-notebook-server_nb-server[Viewing Python packages installed on your notebook server] for details.
